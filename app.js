...
const C={regla:['🎲','REGLA'],combate:['⚔️','COMBATE'],recursos:['🎒','RECURSOS'],pokemon:['🐾','POKÉMON'],caos:['☠️','CAOS']};
const E={
...
function roll(){
 ...
 else if(P.phase==='effect'){let a=E[P.cat][P.al];P.e=a[Math.floor(Math.random()*a.length)];P.steps.push({t:'effect',v:P.e});afterEffect()}
...
function resolveChaos(){if(P&&P.phase==='chaosPrompt'){P.phase='chaos';roll()}}
...
function current(){
 ...
 if(P){
  st.innerHTML=P.steps.map((s,i)=>{
   let t='',v='',ic='🎲',cl='';
   if(s.t==='align'){t='Destino';v=s.v==='positive'?'POSITIVO':'NEGATIVO';ic=s.v==='positive'?'🟢':'🔴';cl=s.v}
   if(s.t==='cat'){t='Tema';v=C[s.v][1];ic=C[s.v][0]}
   if(s.t==='effect'){t='Resultado';v=s.v[0];ic='🎯'}
   if(s.t==='duration'){t='Duración';v=s.v;ic='⏱️'}
   if(s.t==='chaosResult'){t='Resultado de Caos';v=(s.v.al==='positive'?'🟢':'🔴')+' '+C[s.v.c][1]+' → '+s.v.e[0];ic='☠️';cl=s.v.al}
   if(s.t==='gamble'){t='Apuesta';v=s.n+' → '+s.v;ic='🎰';cl=s.n>=5?'positive':s.n<=2?'negative':''}
   return `<div class="die-card ${i===P.steps.length-1?'latest':''}"><div class="die-icon">${ic}</div><div><div class="die-title">${t}</div><div class="die-value ${cl}">${esc(v)}</div></div><div class="die-number">${s.n??''}</div></div>`
  }).join('');
  ...
  if(P.e){
    ...
    a.innerHTML=`<div class="action-card"><strong>☠️ CAOS</strong><span>El Caos no resuelve automáticamente. Pulsa para escoger un resultado aleatorio entre todas las opciones positivas y negativas.</span><div class="action-buttons"><button class="main-btn" onclick="resolveChaos()">🎲 Resolver Caos</button></div></div>`;
  } else if(P.phase==='gamble'){
    a.innerHTML='';
  }
 } else if(S.current){
  let h=S.current;
  st.innerHTML=`<div class="die-card latest"><div class="die-icon">${h.chaos?'☠️':C[h.category][0]}</div><div><div class="die-title">Resultado final</div><div class="die-value ${h.positive?'positive':'negative'}">${esc(h.effect[0])}</div><div class="history-details">${esc(h.effect[1])}</div></div><div class="die-number">${h.chaos?'CAOS':''}</div></div>`;
  a.innerHTML=`<div class="action-card">${h.duration?'<div>⏱️ '+esc(h.duration.value)+'</div>':''}${h.gambleResult?'<div>🎰 '+esc(h.gambleResult)+'</div>':''}<button class="main-btn" onclick="begin()">🎲 Tirar desde 0</button></div>`;
...
function history(){
 ...
 l.innerHTML=S.history.map((h,i)=>`<article class="history-item"><div class="history-head"><strong>#${S.history.length-i} · ${new Date(h.date).toLocaleString('es-ES',{dateStyle:'short',timeStyle:'short'})}</strong><span class="pill ${h.chaos?'chaos':h.positive?'good':'bad'}">${h.chaos?'☠️ CAOS':h.positive?'🟢 POSITIVO':'🔴 NEGATIVO'}</span></div><div class="history-main"><span class="pill">${C[h.category][0]} ${C[h.category][1]}</span><span class="pill">${esc(h.effect[0])}</span>${h.duration?'<span class="pill">⏱️ '+esc(h.duration.value)+'</span>':''}${h.gambleResult?'<span class="pill">🎰 '+esc(h.gambleResult)+'</span>':''}</div><div class="history-details">${esc(h.effect[1])}</div><div class="history-actions"><button class="complete-btn ${h.done?'completed':''}" onclick="toggleDone('${h.id}')">${h.done?'☑ Completado':'☐ Marcar completado'}</button><button class="delete-btn" onclick="delH('${h.id}')">Borrar</button></div></article>`).join('')
}
...
function stats(){document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.dataset.tab===tab));let d=S.stats[tab],rows=Object.entries(d).sort((a,b)=>b[1]-a[1]);document.querySelector('#stats').innerHTML=rows.length?'<div class="stat-box"><h3>'+ (tab==='normal'?'🎲 Tiradas normales':'☠️ Resultados de Caos')+'</h3>'+rows.map(x=>`<div class="stat-row"><span>${esc(x[0])}</span><strong>${x[1]}</strong></div>`).join('')+'</div>':'<div class="empty-state compact">Todavía no hay estadísticas.</div>'}
